 ...
 **/
@RestController
public class CartsController implements ApiApi {
  ...
  public ResponseEntity<List<Cart>> getCartByCustomerId(String customerId) {
    return ok(Collections.singletonList(assembler.toModel(service.getCartByCustomerId(customerId))));
  }
  ...
  public ResponseEntity<List<Item>> getCartItemsByItemId(String customerId, String itemId) {
    return ok(Collections.singletonList(service.getCartItemsByItemId(customerId, itemId)));
  }
}
